...
def path_energy(y, x, U, b_start=None, b_end=None):
    """Calculates the energy of a tag path y for a given input x,
    transition energies U and boundary energies b_start, b_end."""
    x = add_boundary_energy(x, b_start, b_end)
    return path_energy0(y, x, U)


def path_energy0(y, x, U):
...
def sparse_chain_crf_loss(y, x, U, b_start=None, b_end=None):
    """Given the true sparsely encoded tag sequence y, input x,
    ...
    """
    x = add_boundary_energy(x, b_start, b_end)
    energy = path_energy0(y, x, U)
    energy -= free_energy0(x, U)
    return K.expand_dims(-energy, -1)


def chain_crf_loss(y, x, U, b_start=None, b_end=None):
    """Variant of sparse_chain_crf_loss but with one-hot encoded tags y."""
    y_sparse = K.argmax(y, -1)
    y_sparse = K.cast(y_sparse, "int32")
    return sparse_chain_crf_loss(y_sparse, x, U, b_start, b_end)


def add_boundary_energy(x, b_start=None, b_end=None):
    """Given the observations x, it adds the start boundary energy b_start (resp.
    end boundary energy b_end on the start (resp. end) elements.
    """
...
def viterbi_decode(x, U, b_start=None, b_end=None):
...
class ChainCRF(Layer):
    ...
    # Masking
        This layer does NOT support masking for input sequences of variable length.
